	...
	public JType computeReturnType(Transition transition) {
		...
		transition.accept(new TransitionVisitor.$() {
			...
			public void visit(AscendingTransition transition) {
				JClass sc = BUILDER_OR_MARKER_NARROWED_INTERFACE_STRATEGY.createWeakType(ctx, transition.getOwner());
				initialType.set(sc.getTypeParameters().get(0));
			}

			@Override
			public void visit(LateralTransition transition) {
				initialType.set(BUILDER_OR_MARKER_NARROWED_INTERFACE_STRATEGY.createWeakType(ctx, transition.getSibling()));
			}

			@Override
			public void visit(RecursiveTransition transition) {
				initialType.set(BUILDER_OR_MARKER_NARROWED_INTERFACE_STRATEGY.createWeakType(ctx, transition.getSelf()));
			}
		...
		for (int i = sequentialStates.size()-1; i >= 0; --i) {
			...
			JClass targetBuilder = BUILDER_OR_MARKER_INTERFACE_STRATEGY.createWeakType(ctx, sequentialState);
